...
--
-- Loads the real-API-fetched CSVs into the schema created by schema.sql.
-- Data was collected via fetch_real_data.py (yfinance, Yahoo/Google News RSS,
-- NewsAPI, FRED).  Uses psql \copy (client-side) so it works on a clean
-- machine with no server-side superuser file permissions.
--
-- RUN FROM THE PROJECT ROOT (replace <user> with your OS username):
