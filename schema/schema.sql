...
-- Milestone 1 — Schema Design and DDL  (API-audited revision, 50-ticker universe)
-- =============================================================================
...
COMMENT ON TABLE  companies IS
  ...
  '50 tickers across 6 sectors. ticker is the natural candidate key used by all price/news APIs.';
...
-- API source: yf.Ticker(ticker).history(period="2y")
...
-- Row count: 50 tickers × ~501 trading days = 25,050 rows (exceeds 1,000 min).
...
COMMENT ON TABLE  stock_prices IS
  'Daily OHLCV price data. Source: yf.Ticker().history(period="2y"). '
  '50 tickers × 501 trading days = 25,050 rows. '
...
-- API source: yf.Ticker(ticker).upgrades_downgrades  (current yfinance API)
--   Fallback:  yf.Ticker(ticker).recommendations     (deprecated, kept as fallback)
--   DataFrame columns: Firm → analyst_firm, "ToGrade"/"To Grade" → rating, index → rating_date
--   Column name varies between yfinance versions; fetcher tries ToGrade, To Grade, toGrade.
--
-- KNOWN API LIMITATION — target_price_usd:
--   upgrades_downgrades does NOT provide per-firm target prices.
...
COMMENT ON TABLE  analyst_ratings IS
  'Analyst ratings from yf.Ticker().upgrades_downgrades (current API). '
  'Fields: Firm, ToGrade, date. Normalised rating vocabulary enforced by CHECK constraint. '
...
-- FRED series used (8 series, 2022-present):
...
--   DCOILWTICO       : WTI Crude Oil Price                 (daily, USD per Barrel)
--   VIXCLS           : CBOE Volatility Index (VIX)         (daily, Index)
-- Estimated rows: 8 series × ~48 months (varies by frequency) = ~384+ rows.
